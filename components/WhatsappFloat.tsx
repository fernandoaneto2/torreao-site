...
type Props = {
  baseHref: string;
  scrollThreshold?: number; // px antes de aparecer — default 400
};

export default function WhatsappFloat({ baseHref, scrollThreshold = 400 }: Props) {
  ...
  // Scroll trigger: aparece após scrollThreshold px
  useEffect(() => {
    const onScroll = () => setVisible(window.scrollY > scrollThreshold);
    onScroll(); // verifica posição inicial (ex: refresh mid-page)
